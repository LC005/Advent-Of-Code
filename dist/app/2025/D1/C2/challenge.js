...
const mainLoop = () => {
    ...
    fileImporter.fileDataModified.forEach((instruction) => {
        if (instruction.dir === 'L' && instruction.value === '685') {
            console.log('Debug here');
        }
        const oldPosition = positon;
        let value = instruction.value;
        if (value.length > 2) {
            const moveString = value.slice(0, -2);
            const moveAmount = parseInt(moveString);
            targetPositionCounter += moveAmount;
            value = value.slice(-2);
        }
        ...
        if (isNaN(targetPositionCounter)) {
